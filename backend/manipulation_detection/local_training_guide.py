...
class LocalTrainer:
    ...
    def __init__(
        ...
        if class_weights is not None:
            ...
            logger.info("Using weighted loss function for imbalanced classes")
        else:
    ...
    def train_epoch(self):
        ...
        for batch in progress_bar:
            ...
            # Calculate loss using class-weighted loss function
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train Manipulation Detection Model Locally"
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="../dataset_augmented/enhanced_critical_splits_augmented.json",
        help="Path to cleaned/augmented dataset JSON",
    )
    parser.add_argument(
        "--model-name",
        type=str,
    ...
    parser.add_argument(
        "--use-class-weights",
        action="store_true",
        help="Use class weights for imbalanced data (recommended)",
    )
    ...
    # Extract splits from cleaned dataset
    ...
    train_labels = [sample["manipulation_tactic"] for sample in train_data]
    val_texts = [sample["text"] for sample in val_data]
    val_labels = [sample["manipulation_tactic"] for sample in val_data]
    ...
    # Override default label mapping with actual classes
    model.labels = unique_classes
    model.label_to_id = {label: i for i, label in enumerate(unique_classes)}
    model.id_to_label = {i: label for i, label in enumerate(unique_classes)}

    logger.info(f"Number of classes: {num_classes}")
    logger.info(f"Classes: {', '.join(unique_classes)}")
    ...
    if args.use_class_weights:
        ...
        class_weights = torch.tensor(
            [weights_dict[class_name] for class_name in unique_classes],
            dtype=torch.float32,
        )
        logger.info("Class weights (for balancing):")
    ...
    trainer = LocalTrainer(
        model, train_loader, val_loader, device, config, class_weights=class_weights
    )
...
def test_model(model, tokenizer, device):
    ...
    for text in test_texts:
        ...
        logger.info(
            f"Prediction: {result['predicted_label']} (confidence: {result['confidence']:.3f})"
        )
        logger.info("-" * 50)
